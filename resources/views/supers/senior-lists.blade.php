            ...
            <div class="col-sm-12">
              ...
              <div class="card">
                <div class="card-header">
                  <div class="d-flex">
                    <div class="m-r-20">
                      <a class="btn btn-outline-primary btn-sm" id="createSenior" href="javascript:void(0)"> Create Senior</a>
                    </div>
                  </div>
                </div>
...
<div class="modal fade" id="ajaxModel" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="seniorForm" name="seniorForm" class="form-horizontal">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="userForm" name="userForm" class="form-horizontal">
        <div class="modal-body">
          <input type="hidden" name="senior_id_one" id="senior_id_one">
          <div class="form-group">
            <label>Name</label>
...
<script type="text/javascript">
  $(function () {
    ...
    $('body').on('click', '.addTransaction', function () {
      var senior_id_one = $(this).data('id');
      $.get("{{ route('seniors-lists.index') }}" +'/' + senior_id_one +'/edit', function (data) {
        $('#modelHeadingOne').html("စူပါ ထည့်/ထုတ်");
        $('#ajaxModelOne').modal('show');
        $('#senior_id_one').val(data.id);
    ...
    $('#createSenior').click(function () {
      ...
      $('#senior_id').val('');
      $('#seniorForm').trigger("reset");
      $('#modelHeading').html("အကောင့်ဖွင့်ရန်");
      $('#ajaxModel').modal('show');
    });

    $('body').on('click', '.editSenior', function () {
      $('#banned').show();
      var senior_id = $(this).data('id');
      $.get("{{ route('seniors-lists.index') }}" +'/' + senior_id +'/edit', function (data) {
        $('#modelHeading').html("Update Profile");
        $('#saveBtn').val("edit-senior");
        $('#ajaxModel').modal('show');
        $('#senior_id').val(data.id);
        $('#amount').val(data.amount);
        $('#name').val(data.name);
        $('#username').val(data.username);
    ...
    $('#saveBtn').click(function (e) {
      ...
      $(this).html('Sending..');
      $.ajax({
        data: $('#seniorForm').serialize(),
        url: "{{ route('seniors-lists.store') }}",
        type: "POST",
        dataType: 'json',
    ...
    $('body').on('click', '.deleteSenior', function () {
      ...
      $.ajax({
        type: "DELETE",
        url: "{{ route('seniors-lists.store') }}"+'/'+senior_id,
        success: function (data) {
          table.draw();
        },
        error: function (data) {
    ...
    $('body').on('click', '.restoreSenior', function () {
      ...
      swal.fire({
        title: "Unban?",
        icon: 'question',
        text: "Are you sure you want to unban this senior from the banned seniors list?",
        type: "success",
        ...
        if (e.value === true) {
          ...
          $.ajax({
            type: "GET",
            url: "{{ url('super/restore-senior') }}"+'/'+senior_id,
