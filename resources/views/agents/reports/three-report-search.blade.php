...
<script>
  ...
  var table = $('#bets').DataTable({
    ...
    pageLength: 25,
    ajax: {
      url:'{{ route("agent.3d-report-search.index") }}',
      data: function(d) {
    ...
    footerCallback: function ( row, data, start, end, display ) {
      ...
      $( api.column( 4 ).footer() ).html(three_digit_count);
      $( api.column( 5 ).footer() ).html(amount);
      $( api.column( 6 ).footer() ).html(compensation);
      $( api.column( 7 ).footer() ).html(votes);
      $( api.column( 8 ).footer() ).html(diffColor);
    },
  });
